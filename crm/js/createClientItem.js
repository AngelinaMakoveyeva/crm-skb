...
export const createClientItem = (data) => {
  const clientTR = document.createElement('tr'),
    ...
    changeClient = changeClientModal(data),
    changeSpinner = document.createElement('span'),
    deleteSpinner = document.createElement('span');

  clientTR.classList.add('clients__item');
  idTD.classList.add('clients__column');
  clientId.classList.add('clients__id');
  fioTD.classList.add('clients__column');
  clientFio.classList.add('clients__fio');
  dateTD.classList.add('clients__column');
  createDate.classList.add('clients__date--create');
  createTime.classList.add('clients__time--create');
  changeTD.classList.add('clients__column');
  changeDate.classList.add('clients__date--change');
  changeTime.classList.add('clients__time--change');
  contactsTD.classList.add('clients__column');
  contactList.classList.add('clients__list');
  actionsTD.classList.add('clients__column');
  changeBtn.classList.add('clients__btn--change', 'btn-reset');
  deleteBtn.classList.add('clients__btn--delete', 'btn-reset');
  changeSpinner.classList.add('clients__spinner');
  deleteSpinner.classList.add('clients__spinner');

  clientTR.setAttribute('id', `${data.id}`)
  clientId.textContent = data.id.substr(0, 6);
  clientFio.textContent = data.surname + ' ' + data.name + ' ' + data.lastName;
  ...
  changeSpinner.innerHTML = `<svg width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  for (const contact of data.contacts) {
    createContactItemByType(contact.type, contact.value, contactList);
  };
  ...
  deleteBtn.addEventListener('click', () => {
    ...
    deleteBtn.classList.add('clients-wait');
    ...
    setTimeout(() => {
      ...
      deleteBtn.classList.remove('clients-wait');
    }, 1500);
  });

  changeBtn.addEventListener('click', () => {
    changeSpinner.style.display = 'block';
    changeBtn.classList.add('clients-wait');

    setTimeout(() => {
      document.body.append(changeClient.changeModal);

      changeSpinner.style.display = 'none';
      changeBtn.classList.remove('clients-wait');
    }, 1500);
  });

  idTD.append(clientId);
  fioTD.append(clientFio);
  dateTD.append(createDate, createTime);
  changeTD.append(changeDate, changeTime);
  contactsTD.append(contactList);
  changeBtn.append(changeSpinner);
  deleteBtn.append(deleteSpinner);
  actionsTD.append(changeBtn, deleteBtn);
  clientTR.append(idTD, fioTD, dateTD, changeTD, contactsTD, actionsTD);
...
function formatDate(data) {
  const newDate = new Date(data),
    ...
    resultDate = newDate.toLocaleString('ru', correctDate);
...
function formatTime(data) {
  const newDate = new Date(data),
    ...
    resultTime = newDate.toLocaleString('ru', correctTime);

  return resultTime;
};

// Реализуем иконки контактов
...
function createContactItemByType(type, value, item) {
  switch (type) {
    case 'Телефон':
      let tel;
      const svgTel = `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g opacity="0.7">
        <circle cx="8" cy="8" r="8" fill="#9873FF"/>
        <path d="M11.56 9.50222C11.0133 9.50222 10.4844 9.41333 9.99111 9.25333C9.83556 9.2 9.66222 9.24 9.54222 9.36L8.84444 10.2356C7.58667 9.63556 6.40889 8.50222 5.78222 7.2L6.64889 6.46222C6.76889 6.33778 6.80444 6.16444 6.75556 6.00889C6.59111 5.51556 6.50667 4.98667 6.50667 4.44C6.50667 4.2 6.30667 4 6.06667 4H4.52889C4.28889 4 4 4.10667 4 4.44C4 8.56889 7.43556 12 11.56 12C11.8756 12 12 11.72 12 11.4756V9.94222C12 9.70222 11.8 9.50222 11.56 9.50222Z" fill="white"/>
        </g>
        </svg>`;
      createContactLink(type, value, tel, svgTel, item);
      break;
    case 'Facebook':
      let fb;
